...
const SearchBar = () => { 
  ...
  return ( 
    <div className="relative z-30 px-4 pt-4 md:pt-22 font-[Outfit]"> 
      ...
      <div className="max-w-6xl mx-auto bg-gray-200/20 
backdrop-blur-md p-4 flex flex-wrap items-center gap-3 justify-center 
rounded-[40px] border border-white/20 shadow-2xl"> 
         
        {/* Selector de Acción adaptado a móviles */} 

        <div className="flex flex-wrap sm:flex-nowrap bg-black/60 
p-1 rounded-xl border border-white/5 justify-center gap-1 w-full 
sm:w-auto"> 
          {["Comprar", "Arrendar", "Vender"].map((accion) => ( 
            <button 
              ...
              className={`px-4 sm:px-6 py-3 rounded-lg text-xs 
sm:text-sm font-medium transition-all duration-300 whitespace-nowrap ${ 
        ...
        <div className="relative w-full sm:w-56" ref={dropdownRef}> 
          <button 
            ...
            className="px-5 py-4 bg-gray-600/60 text-white rounded-xl 
flex items-center justify-between w-full border border-white/10" 
          > 
            <span className="text-sm truncate">{tipoPropiedad?.label || "Tipo de propiedad"}</span> 

            <ChevronDown size={16} className={`text-[#24B6C1] 
transition-transform ${openDropdown ? "rotate-180" : ""}`} /> 
          </button> 
...
bg-[#1a1a1a]/95 backdrop-blur-2xl p-4 shadow-2xl w-full sm:w-72 border 
border-white/10 rounded-2xl z-50 max-h-72 overflow-y-visible 
sm:overflow-visible overflow-x-visible" 
              > 
...
rounded-xl hover:bg-[#24B6C1]/10 text-white/80 transition flex 
...
font-bold text-[#24B6C1] mb-3 px-2 py-1 hover:underline uppercase 
tracking-wider" 
                      > 
                        ← Volver atrás 
                      </button> 
...
justify-between px-4 py-3 rounded-lg cursor-pointer text-sm 
text-gray-300 hover:bg-[#24B6C1]/10 hover:text-[#24B6C1] 
whitespace-nowrap" 
                          > 

                            {sub.label} {tipoPropiedad?.id === sub.id 
&& <Check size={14} className="text-[#24B6C1]" />} 
                          </li> 
                        ))} 
                      </ul> 
                    </div> 
...
group-hover:text-[#24B6C1] 
transition-colors">{prop.nombre}</span> 

                          <ChevronRight size={14} 
className="text-gray-400 group-hover:text-[#24B6C1] transition-colors" 
/> 
                        </div> 
...
absolute left-full top-0 ml-2 bg-[#1a1a1a]/95 backdrop-blur-2xl p-2 
shadow-2xl w-60 border border-white/10 rounded-2xl z-50"> 
                          <ul className="space-y-0.5"> 
                            {prop.sub.map((sub, j) => ( 
                              <li key={j}  
                                onClick={(e) => {  
...
justify-between px-4 py-3 rounded-lg cursor-pointer text-sm 
text-gray-300 hover:bg-[#24B6C1]/10 hover:text-[#24B6C1] 
hover:translate-x-1 transition-all duration-300 whitespace-nowrap" 
                              > 

                                {sub.label} {tipoPropiedad?.id === 
sub.id && <Check size={14} className="text-[#24B6C1]" />} 
                              </li> 
                            ))} 
                          </ul> 
                        </div> 
        ...
        <div className="flex-1 min-w-[240px] w-full sm:w-auto relative" ref={suggestionRef}> 
          <input 
            ...
            className="w-full px-6 py-4 bg-gray-400/90 text-white 
rounded-xl placeholder-white/90 focus:outline-none focus:ring-1 
focus:ring-[#24B6C1] text-sm" 
          /> 
...
bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl z-50 max-h-60 
overflow-y-auto" 
              > 
                {filteredComunas.map((c, index) => ( 
                  <div key={`${c.id}-${index}`} onClick={() => { 
                    ...
                    className="px-6 py-3 hover:bg-[#24B6C1]/20 
cursor-pointer text-sm text-gray-300 flex items-center justify-between" 
                  > 
        ...
        <button onClick={handleSearch} 
          ...
          className="w-full sm:w-auto px-8 py-4 bg-[#24B6C1] 
hover:bg-cyan-600 text-white rounded-xl flex items-center justify-center
 gap-2 transition-all group shadow-lg" 
        > 
