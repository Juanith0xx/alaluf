...
const VALOR_USD_REFERENCIAL = 950;   // Valor aproximado del dólar
...
const MortgageCalculator = () => {
  ...
  // 🌟 MEJORA: Atenuamos la imagen sutilmente (0.15 de opacidad). El mármol se ve blanco pero controlado.
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-[Outfit]">
      ...
      {/* HEADER DE LA PÁGINA */}
      <div className="bg-[#111111] border-b border-white/5 pt-32 pb-6 shadow-xl relative z-20">
        <div className="max-w-[1600px] mx-auto px-6 flex items-center gap-8">
          <button onClick={() => navigate(-1)} className="p-4 border border-white/10 rounded-2xl hover:bg-[#24B6C1] transition-all">
            <FaArrowLeft />
          </button>
          <h1 className="text-xl lg:text-3xl font-black tracking-tighter uppercase italic">
      ...
      <div style={fullSectionStyle} className="relative py-6 min-h-screen">
        
        {/* CONTENIDO CENTRAL */}
        <div className="max-w-[1600px] mx-auto px-6 grid lg:grid-cols-12 gap-10 relative z-10">
          
          {/* COLUMNA IZQUIERDA: CONTROLES / SLIDERS */}
          <div className="lg:col-span-7 space-y-8 bg-black/70 backdrop-blur-xl p-8 rounded-[40px] border border-white/10 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col focus-within:border-[#24B6C1] focus-within:ring-1 focus-within:ring-[#24B6C1] transition-all relative">
                <div className="flex items-baseline justify-between">
                  <div className="flex items-center w-full">
                    {monedaActiva === "CLP" && <span className="text-3xl font-black text-[#24B6C1] mr-1">$</span>}
                    {monedaActiva === "USD" && <span className="text-3xl font-black text-[#24B6C1] mr-1">US$</span>}
                    <input 
                      type="number"
                      value={inputValue || ""}
                      onChange={(e) => handleInputChange(Number(e.target.value))}
                      placeholder="0"
                      className="bg-transparent text-4xl font-black text-white w-full outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    {monedaActiva === "UF" && <span className="text-2xl font-black text-[#24B6C1] ml-2">UF</span>}
                  </div>
                </div>

                {/* Equivalencias secundarias */}
                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-4 pt-3 border-t border-white/5 text-xs text-gray-400 font-medium">
              ...
              <div className="flex justify-between text-[10px] text-gray-600 font-bold">
                <span>1.000 UF</span>
                <span>Rango de simulación flexible</span>
                <span>40.000 UF</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-2xl font-black text-[#24B6C1] italic">
                  {porcentajePie}% <span className="text-sm font-normal text-white/60">({formatUF(montoPie)} UF)</span>
                </span>
              </div>
              <input 
              ...
              <div className="flex justify-between text-[10px] text-gray-600 font-bold">
                <span>10% Mínimo</span>
                <span>Monto Pie: {formatCLP(montoPie * VALOR_UF_REFERENCIAL)}</span>
                <span>50% Máximo</span>
              </div>
            </div>

            {/* CONTROL 3: PLAZO DE 5 A 40 AÑOS */}
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
                {[5, 10, 15, 20, 25, 30, 35, 40].map((anios) => (
                  <button
                    ...
                    className={`py-3.5 rounded-xl font-bold transition-all text-xs ${
                    plazoAnios === anios 
            ...
            <div className="space-y-4 pt-2">
              <div className="flex justify-between items-end">
                ...
                <span className="text-2xl font-black text-[#24B6C1] italic">
                  {tasaInteres}% <span className="text-sm font-normal text-white/60">Anual</span>
                </span>
              </div>
              <input 
              ...
              <div className="flex justify-between text-[10px] text-gray-600 font-bold">
                <span>2.0%</span>
                <span>Promedio de mercado actual</span>
                <span>7.0%</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#111111] to-[#1a1a1a] rounded-[40px] p-8 border border-white/10 relative overflow-hidden shadow-2xl">
              ...
              <div className="border-t border-white/10 mt-6 pt-6 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-lg font-black text-white italic">{formatUF(montoCredito)} UF</span>
                </div>
                <div>
                  <span className="text-[9px] uppercase font-bold text-gray-400 block tracking-wider">Porcentaje de Financiamiento</span>
                  <span className="text-lg font-black text-white italic">{100 - porcentajePie}%</span>
                </div>
              </div>
            </div>

            {/* CUADRO DE REQUISITO DE RENTA MÍNIMA */}
            <div className="bg-white text-black rounded-[40px] p-8 shadow-2xl relative overflow-hidden">
              <span className="text-[10px] uppercase font-bold tracking-widest text-gray-400 block mb-1">Renta Mínima Sugerida</span>
              <h3 className="text-3xl font-black tracking-tighter text-gray-900 italic">
