...
import { useNavigate } from 'react-router-dom'; // <-- Importación para navegación SPA
...
const Licitaciones = () => {
  ...
  // Datos Estructurados (Schema Markup) enfocados en Servicios de Licitación Inmobiliaria
  ...
  return (
    <>
      <Helmet>
        ...
        <title>Licitaciones Inmobiliarias | Venta de Activos de Alto Valor</title>
        <meta name="description" content="Maximiza el valor de tu propiedad con nuestro proceso de licitación inmobiliaria. Acceso a terrenos y edificios exclusivos que no llegan al mercado abierto." />
        ...
        <meta name="twitter:title" content="Licitaciones de Activos | Oportunidades Exclusivas" />
        <meta name="twitter:description" content="Los mejores activos no se publican, se consiguen. Accede a licitaciones inmobiliarias en Chile." />
        {/* <meta name="twitter:image" content="https://www.tudominio.cl/assets/licitaciones.png" /> */}
      ...
      >
        ...
        <section className="relative w-full h-[250px] sm:h-[300px] md:h-[350px] lg:h-[400px] overflow-hidden border-b border-white/10">
          ...
          <img 
            ...
            alt="Edificios y activos de alto valor para licitación inmobiliaria" // <-- Alt optimizado
        ...
        <section className="py-12 md:py-16 px-4 sm:px-6 md:px-20 relative z-10">
          <div className="container mx-auto max-w-7xl">
            <div className="bg-[#0091A4] text-center text-white rounded-2xl p-6 sm:p-10 md:p-14 flex flex-col items-center shadow-2xl transition-transform hover:scale-[1.01]">
              ...
              {/* Botón CTA actualizado con useNavigate */}
              <button 
                onClick={() => navigate('/contacto')} // ⚠️ Cambia '/contacto' por la ruta donde recibas a los inversionistas
                className="w-full sm:w-fit text-center bg-white text-[#0091A4] hover:bg-gray-100 font-bold text-base md:text-lg px-8 sm:px-12 py-3.5 md:py-4 rounded-xl shadow-md transition duration-300 active:scale-95"
              >
                Quiero acceder
              </button>
            </div>
